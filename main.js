...
function crosswordSolver(crossword, words) {
    ...
    const invalidWords = words.length < 3 || !Array.isArray(words) || words.some((word) => typeof word !== "string")
            ...
            for (let colInd = 0; colInd < grid[0].length; colInd++) {
                ...
                function directOfWord(direction) {
                    ...
                    for (let i = 0; i < word.length; i++) {
                        ...
                        if (
                            row >= grid.length ||
                            col >= grid[0].length || (puzzleWords[row][col] !== "" && puzzleWords[row][col] !== word[i])
                        ) {
                            break
                        }
